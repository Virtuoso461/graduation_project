...
const loadExamDetail = async () => {
  try {
    ...
    exam.value = {
      ...
      type: 'midterm',
      status: 'upcoming',
      description: '本次测试涵盖数据结构基础知识，包括线性表、栈、队列和树的基本操作与应用。请在规定时间内完成所有题目，并确保答案的完整性和正确性。测试结果将作为期中考核的重要部分，占总成绩的30%。',
      duration: 90,
      questionCount: 30,
      totalScore: 100,
      ...
      isProctored: false,
      composition: [
        { type: 'single', count: 15, points: 2 },
        { type: 'multiple', count: 5, points: 4 },
        { type: 'truefalse', count: 5, points: 2 },
        { type: 'fillblank', count: 3, points: 5 },
        { type: 'shortanswer', count: 2, points: 10 }
      ]
    }
  } catch (error) {
...
const startExam = () => {
  if (exam.value.status === 'ended') {
    ElMessage.warning('此考试已经结束，无法参加')
    return
  }
  
  ElMessageBox.confirm(
    `您即将开始 ${exam.value.title}，考试时长 ${exam.value.duration} 分钟。开始后计时器将启动，无法暂停。确认现在开始吗？`,
    '开始考试',
    {
    ...
    router.push(`/exams/${examId.value}/take`)
  }).catch(() => {
    // 用户取消操作
  })
}
