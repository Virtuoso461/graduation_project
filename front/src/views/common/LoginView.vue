...
const handleLogin = async () => {
  ...
  try {
    ...
    if (success) {
      // 获取用户实际角色（从API返回的数据中获取）
      const userRole = userStore.userInfo?.role || ''
      console.log('API返回的用户角色:', userRole)

      // 根据实际角色决定重定向路径
      switch(userRole) {
        case 'ADMIN':
          console.log('管理员角色，准备跳转到: /admin/dashboard')
          router.push('/admin/dashboard')
          break
        case 'TEACHER':
          console.log('教师角色，准备跳转到: /teacher/dashboard')
          router.push('/teacher/dashboard')
          break
        case 'STUDENT':
        default:
          console.log('学生角色，准备跳转到: /student/dashboard')
